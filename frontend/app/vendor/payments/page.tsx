...
const getPaymentBadge = (status: string) => {
    switch (status) {
        case 'PAYMENT_SUCCESS': return <span className="inline-flex items-center rounded-full bg-emerald-50 px-2 py-1 text-xs font-medium text-emerald-700 ring-1 ring-emerald-600/20"><ShieldCheck size={12} className="mr-1" /> Released</span>;
        case 'ORDER_CREATED':
        case 'PAYMENT_PENDING':
        case 'PAYMENT_VERIFICATION_PENDING':
            return <span className="inline-flex items-center rounded-full bg-amber-50 px-2 py-1 text-xs font-medium text-amber-700 ring-1 ring-amber-600/20"><Clock size={12} className="mr-1" /> In Escrow</span>;
        case 'PAYMENT_FAILED':
        case 'CANCELLED':
            return <span className="inline-flex items-center rounded-full bg-red-50 px-2 py-1 text-xs font-medium text-red-700 ring-1 ring-red-600/20"><XCircle size={12} className="mr-1" /> Cancelled</span>;
        case 'REFUNDED':
        ...
        <DashboardLayout>
            <div className="space-y-6">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div>
                        <h1 className="text-2xl font-bold text-slate-900">Vendor Payouts & Disbursements</h1>
                        <p className="text-sm text-slate-500 mt-1">Track payments received and funds released from escrow vaults.</p>
                    </div>
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-left text-sm">
                            <thead className="bg-slate-50 border-b border-slate-200">
                                <tr>
                                    <th className="p-4 font-semibold text-slate-600">Disbursement Ref</th>
                                    ...
                                    <th className="p-4 font-semibold text-slate-600">Date Processed</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-slate-100">
                                {isLoading && <tr><td colSpan={5} className="p-8 text-center text-slate-500">Loading payout records...</td></tr>}
                                {!isLoading && payments.length === 0 && (
                                    <tr><td colSpan={5} className="p-8 text-center text-slate-500">No payout records found.</td></tr>
                                )}
